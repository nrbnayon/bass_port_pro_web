import {
  LakeCard,
  CatchCard,
  FeatureCard,
  ReportCard,
} from "@/types/landingData.types";

export const navLinks = [
  { label: "Home", href: "#home" },
  { label: "Lakes", href: "#lakes" },
  { label: "BassPort", href: "#catches" },
  { label: "Reports", href: "#reports" },
  { label: "Contact", href: "#footer" },
];

export const heroStats = [
  { id: "1", value: "12+", label: "Premium Lakes" },
  { id: "2", value: "500+", label: "Catch Photos" },
  { id: "3", value: "1,200+", label: "Fishing Reports" },
  { id: "4", value: "5,000+", label: "Active Anglers" },
];

export const lakes: LakeCard[] = [
  {
    id: "guntersville",
    name: "Lake Guntersville",
    state: "Alabama",
    rating: 4.9,
    temp: "72F",
    weather: "Clear",
    wind: "4.2 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Striped Bass"],
    image: "/images/lake1.jpg",
    color: "from-cyan-700/80 to-sky-900/80",
    size: 69100,
    catchRate: 4.2,
    recordBass: 13.5,
    description: "Lake Guntersville is widely regarded as one of the best bass fishing lakes...",
    condition: "Excellent",
    clarity: "Clear",
  },
  {
    id: "rayburn",
    name: "Sam Rayburn Reservoir",
    state: "Texas",
    rating: 4.8,
    temp: "68F",
    weather: "Stained",
    wind: "3.1 mph",
    species: ["Largemouth Bass", "Spotted Bass", "Crappie"],
    image: "/images/lake2.jpg",
    color: "from-emerald-700/80 to-teal-900/80",
    size: 114500,
    catchRate: 3.8,
    recordBass: 16.8,
    description: "One of the premier bass fishing destinations in Texas. Sam Rayburn...",
    condition: "Excellent",
    clarity: "Stained",
  },
  {
    id: "chickamauga",
    name: "Lake Chickamauga",
    state: "Tennessee",
    rating: 4.8,
    temp: "68F",
    weather: "Clear",
    wind: "6.0 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Striped Bass"],
    image: "/images/lake3.jpg",
    color: "from-blue-700/80 to-cyan-900/80",
    size: 36240,
    catchRate: 4.0,
    recordBass: 15.2,
    description: "Chickamauga has exploded onto the bass fishing scene with incredible...",
    condition: "Excellent",
    clarity: "Clear",
  },
  {
    id: "fork",
    name: "Lake Fork",
    state: "Texas",
    rating: 4.7,
    temp: "65F",
    weather: "Clear",
    wind: "5.2 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Crappie", "Catfish"],
    image: "/images/lake4.jpg",
    color: "from-emerald-800/80 to-green-950/80",
    size: 27264,
    catchRate: 3.2,
    recordBass: 18.18,
    description: "Lake Fork holds the Texas state record Largemouth bass and has...",
    condition: "Excellent",
    clarity: "Clear",
  },
  {
    id: "tohpekaliga",
    name: "Lake Tohpekaliga",
    state: "Florida",
    rating: 4.7,
    temp: "72F",
    weather: "Stained",
    wind: "4.5 mph",
    species: ["Largemouth Bass", "Bluegill", "Crappie"],
    image: "/images/lake5.jpg",
    color: "from-slate-700/80 to-blue-950/80",
    size: 22700,
    catchRate: 3.8,
    recordBass: 16.5,
    description: "Lake Toho is one of Central Florida's premier bass fishing lakes. Recent...",
    condition: "Excellent",
    clarity: "Stained",
  },
  {
    id: "okeechobee",
    name: "Lake Okeechobee",
    state: "Florida",
    rating: 4.6,
    temp: "74F",
    weather: "Stained",
    wind: "3.5 mph",
    species: ["Largemouth Bass", "Bluegill", "Crappie"],
    image: "/images/lake6.jpg",
    color: "from-indigo-700/80 to-sky-900/80",
    size: 450000,
    catchRate: 4.5,
    recordBass: 12.0,
    description: "The Big O is Florida's largest lake and one of the most famous bass fishing...",
    condition: "Good",
    clarity: "Stained",
  },
  {
    id: "falcon",
    name: "Falcon Lake",
    state: "Tennessee",
    rating: 4.6,
    temp: "70F",
    weather: "Clear",
    wind: "4.7 mph",
    species: ["Largemouth Bass", "Striped Bass", "Catfish"],
    image: "/images/lake7.jpg",
    color: "from-blue-800/80 to-blue-950/80",
    size: 83654,
    catchRate: 3.0,
    recordBass: 14.5,
    description: "Falcon lake on the Texas-Mexico border is renowned for producing...",
    condition: "Fair",
    clarity: "Clear",
  },
  {
    id: "fork_2",
    name: "Lake Fork",
    state: "Texas",
    rating: 4.5,
    temp: "62F",
    weather: "Stained",
    wind: "4.7 mph",
    species: ["Largemouth Bass", "Catfish", "Crappie"],
    image: "/images/lake8.jpg",
    color: "from-blue-600/80 to-blue-900/80",
    size: 27264,
    catchRate: 3.5,
    recordBass: 18.18,
    description: "Lake Fork holds the Texas state record Largemouth bass and has...",
    condition: "Good",
    clarity: "Stained",
  },
  {
    id: "pickwick",
    name: "Pickwick Lake",
    state: "Alabama",
    rating: 4.5,
    temp: "64F",
    weather: "Clear",
    wind: "4.2 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Spotted Bass"],
    image: "/images/lake9.jpg",
    color: "from-blue-700/80 to-blue-950/80",
    size: 43100,
    catchRate: 4.2,
    recordBass: 13.0,
    description: "Pickwick lake offers some of the best smallmouth bass fishing in South...",
    condition: "Good",
    clarity: "Clear",
  },
  {
    id: "stclair",
    name: "Lake St. Clair",
    state: "Michigan",
    rating: 4.4,
    temp: "70F",
    weather: "Clear",
    wind: "5.0 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Muskie"],
    image: "/images/lake10.jpg",
    color: "from-blue-600/80 to-blue-900/80",
    size: 275000,
    catchRate: 5.0,
    recordBass: 10.0,
    description: "Lake St. Clair is the smallmouth bass capital of the world. Crystal clear...",
    condition: "Good",
    clarity: "Clear",
  },
  {
    id: "santeecooper",
    name: "Santee Cooper Lakes",
    state: "South Carolina",
    rating: 4.4,
    temp: "74F",
    weather: "Stained",
    wind: "4.5 mph",
    species: ["Largemouth Bass", "Striped Bass", "Catfish"],
    image: "/images/lake11.jpg",
    color: "from-blue-500/80 to-blue-800/80",
    size: 110000,
    catchRate: 4.5,
    recordBass: 13.8,
    description: "The Big O is Florida's largest lake and one of the most famous bass fishing...",
    condition: "Good",
    clarity: "Stained",
  },
  {
    id: "erie",
    name: "Lake Erie",
    state: "Ohio",
    rating: 4.3,
    temp: "62F",
    weather: "Stained",
    wind: "3.5 mph",
    species: ["Largemouth Bass", "Smallmouth Bass", "Walleye"],
    image: "/images/lake12.jpg",
    color: "from-blue-400/80 to-blue-700/80",
    size: 6361000,
    catchRate: 3.5,
    recordBass: 10.12,
    description: "Lake Fork holds the Texas state record Largemouth bass and has...",
    condition: "Good",
    clarity: "Stained",
  },
];

export const catches: CatchCard[] = [
  {
    id: "c1",
    angler: "FalconFisher",
    lake: "Falcon Lake",
    weight: "13.1 lbs",
    image: "/images/trophy_c1.png",
  },
  {
    id: "c2",
    angler: "ClearLakeking",
    lake: "Clear Lake",
    weight: "11.3 lbs",
    image: "/images/trophy_c2.png",
  },
  {
    id: "c3",
    angler: "FloridaAngler",
    lake: "Lake Okeechobee",
    weight: "10.2 lbs",
    image: "/images/trophy_c3.png",
  },
  {
    id: "c4",
    angler: "ChickamaugaChamp",
    lake: "Lake Chicksmuga",
    weight: "9.4 lbs",
    image: "/images/trophy_c4.png",
  },
];

export const features: FeatureCard[] = [
  {
    id: "f1",
    title: "Lake Intelligence",
    description:
      "Everything you need to fish smarter, catch bigger, and connect with the bass fishing community.",
    icon: "map",
  },
  {
    id: "f2",
    title: "Expert Reports",
    description:
      "Detailed fishing reports from experienced anglers with techniques, patterns, and conditions.",
    icon: "chart",
  },
  {
    id: "f3",
    title: "BassPorn Gallery",
    description:
      "Share your trophy catches with the community. Upload photos with weight, length, and technique details.",
    icon: "camera",
  },
  {
    id: "f4",
    title: "Lake Reviews",
    description:
      "Read honest reviews from fellow anglers and rate your favorite fishing destinations.",
    icon: "message",
  },
  {
    id: "f5",
    title: "Seasonal Patterns",
    description:
      "Understand seasonal bass behavior with best times, techniques, and depth patterns.",
    icon: "calendar",
  },
  {
    id: "f6",
    title: "Record Tracking",
    description:
      "Track your personal bests, total catches, and compete with the BASSPORT community.",
    icon: "target",
  },
];

export const reports: ReportCard[] = [
  {
    id: "r1",
    angler: "TournamentPro",
    date: "2026-02-23",
    lake: "Falcon Lake",
    score: "72%",
    temp: "72°F",
    catches: "18 catches",
    text: "Incredible day on Guntersville! The grass is loaded with fish. Started Flipping mats early and switched to swim jigs mid-morning. Topwater bite turned on after 2 PM near the southern bridge pylons.",
    tags: ["Swim Jig", "Flipping"],
    avatarColor: "bg-[#FF6B35]",
    biggestCatch: "8.2 lbs",
    weather: "Sunny",
    waterLevel: "Normal",
  },
  {
    id: "r2",
    angler: "BigBassHunter",
    date: "2026-02-22",
    lake: "Lake Fork",
    score: "90%",
    temp: "58°F",
    catches: "8 catches",
    text: "Pre-spawn bite is starting to heat up. Fish are moving up from winter holes to staging areas. Jerk baits in 8-12ft producing quality bites. Carolina rig in the deeper flats also picked up a few tanks.",
    tags: ["Jerk bait", "Carolina Rig"],
    avatarColor: "bg-[#3060D9]",
    biggestCatch: "9.5 lbs",
    weather: "Overcast",
    waterLevel: "High",
  },
  {
    id: "r3",
    angler: "FloridaAngler",
    date: "2026-02-21",
    lake: "Lake Okeechobee",
    score: "74%",
    temp: "74°F",
    catches: "22 catches",
    text: "The Big O is fishing phenomenally right now. Flipping heavy mats with a 1oz tungsten weight and creature bait. Frog bite is insane in the Monkey Box area during the early morning hours.",
    tags: ["Flipping", "Frogs"],
    avatarColor: "bg-[#22C55E]",
    biggestCatch: "7.4 lbs",
    weather: "Humid",
    waterLevel: "Low",
  },
  {
    id: "r4",
    angler: "LunkerSlayer",
    date: "2026-02-20",
    lake: "Lake Guntersville",
    score: "85%",
    temp: "65°F",
    catches: "12 catches",
    text: "Found a school of big ones on a deep ledge. Crankbaits were the ticket today. The water is clearing up nicely after last week's rain.",
    tags: ["Crankbaits", "Ledges"],
    avatarColor: "bg-[#A855F7]",
    biggestCatch: "7.9 lbs",
    weather: "Clear",
    waterLevel: "Rising",
  },
  {
    id: "r5",
    angler: "TexasRigKing",
    date: "2026-02-19",
    lake: "Sam Rayburn Reservoir",
    score: "68%",
    temp: "70°F",
    catches: "15 catches",
    text: "Solid day using the classic Texas rig around timber. Most fish were in the 3-4 lb range but managed one solid kicker late in the afternoon.",
    tags: ["Texas Rig", "Timber"],
    avatarColor: "bg-[#EF4444]",
    biggestCatch: "6.8 lbs",
    weather: "Windy",
    waterLevel: "Stable",
  },
  {
    id: "r6",
    angler: "RayburnRunner",
    date: "2026-02-18",
    lake: "Sam Rayburn Reservoir",
    score: "82%",
    temp: "69°F",
    catches: "20 catches",
    text: "Lipless crankbaits over grass flats was the winning pattern today. They are aggressive and hitting hard!",
    tags: ["Lipless", "Grass"],
    avatarColor: "bg-[#F59E0B]",
    biggestCatch: "5.5 lbs",
    weather: "Partly Cloudy",
    waterLevel: "Normal",
  },
];

export const footerLinks = {
  quick: [
    { label: "Home", href: "#home" },
    { label: "Lake Database", href: "#lakes" },
    { label: "BassPort Gallery", href: "#catches" },
    { label: "Fishing Reports", href: "#reports" },
    { label: "Contact Us", href: "#footer" },
  ],
  lakes: [
    { label: "Lake Guntersville", href: "#lakes" },
    { label: "Lake Fork", href: "#lakes" },
    { label: "Sam Rayburn", href: "#lakes" },
    { label: "Lake Okeechobee", href: "#lakes" },
    { label: "Lake Chickamauga", href: "#lakes" },
  ],
};
