...
export default function LakeReportsList({ lake }: LakeReportsListProps) {
  ...
  const reports = allReports.filter((r) => r.lake.toLowerCase().includes(lake.name.toLowerCase()) || lake.name.toLowerCase().includes(r.lake.toLowerCase()));
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
               <div className={`flex h-12 w-12 items-center justify-center rounded-full ${report.avatarColor || 'bg-primary'} text-white`}>
                <span className="text-lg font-bold">{report.angler.charAt(0)}</span>
              </div>
              <div>
                <h3 className="text-lg font-bold text-gray-900 leading-none mb-1.5">{report.angler}</h3>
            ...
            <div className="flex items-center gap-6">
               <div className="flex flex-col items-end">
                <span className="text-lg font-extrabold text-primary">{report.catches}</span>
                <span className="text-[12px] font-bold uppercase tracking-wider text-gray-400 leading-none">Biggest: 7.8 lbs</span>
              </div>
            </div>
          </div>
          
          <div className="mt-8 border-t border-gray-50 pt-8">
            <p className="text-sm font-medium leading-relaxed text-gray-600 line-clamp-3">
              {report.text}
            </p>
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
               {report.tags.map(tag => (
                  <span key={tag} className="rounded-full bg-gray-100 px-3 py-1 text-xs font-bold text-gray-500 uppercase tracking-tight">
                    {tag}
                  </span>
               ))}
            </div>
