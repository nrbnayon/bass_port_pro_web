...
export default function LakeReviewsList() {
  return (
    <div className="flex flex-col gap-10">
      <section className="rounded-2xl border border-gray-100 bg-white p-6 md:p-8 shadow-xs">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <button key={star} className="h-8 w-8 text-[#F59E0B] transition-transform hover:scale-110 active:scale-95">
      ...
      <section>
        <h2 className="text-xl font-bold tracking-tight text-gray-900 md:text-2xl mb-8">
            ...
            >
              <div className="flex items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-full ${review.color} text-white`}>
                    <span className="text-lg font-bold">{review.avatar}</span>
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 leading-none mb-1.5">{review.author}</h3>
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">{review.date}</span>
                  </div>
                </div>
              ...
              <div className="mt-4 border-t border-gray-50 pt-6">
                <p className="text-[15px] font-medium leading-relaxed text-gray-600 italic">
                   &quot;{review.text}&quot;
                </p>
              </div>
            </motion.div>
